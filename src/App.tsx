...
import { 
  ...
  FileText, X, Save, Clock, User, UserCheck, LogOut
} from 'lucide-react'
import { 
...
export default function App() {
  ...
  const TicketTimer = ({ createdAt, priority, status }) => {
    ...
    useEffect(() => {
      ...
      const calculateTime = () => {
        ...
        if (diff <= 0) {
          setIsOverdue(true); setTimeLeft('ATRASADO')
        } else {
          setIsOverdue(false)
  ...
  const stats = useMemo(() => {
    ...
    return { total, open, solved, statusCount }
  }, [tickets])
  ...
  // --- TABELA ---
  const TicketTable = ({ filter, showClaim = false }) => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-gray-400">
            <thead className="bg-[#0f172a] text-gray-200 uppercase font-bold text-xs tracking-wider">
              <tr><th className="px-6 py-4">Status</th><th className="px-6 py-4">Solicitante</th><th className="px-6 py-4">Técnico</th><th className="px-6 py-4">Obs.</th><th className="px-6 py-4 text-center">Ações</th></tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {filteredTickets.length === 0 ? (<tr><td colSpan="5" className="p-12 text-center text-gray-500">Nenhum registro.</td></tr>) : (
                filteredTickets.map((ticket) => (
                  <tr key={ticket.id} className="hover:bg-white/5 transition-colors cursor-pointer" onClick={() => { setEditingTicket({...ticket}); setModalOpen(true); }}>
                    <td className="px-6 py-4 space-y-2">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border ${ticket.status === 'Aberto' ? 'bg-green-500/10 text-green-400 border-green-500/20' : ticket.status === 'Em Andamento' ? 'text-yellow-400 border-yellow-500/20' : 'bg-gray-500/10 text-gray-400 border-gray-500/20'}`}>{ticket.status}</span>
                      <TicketTimer createdAt={ticket.created_at} priority={ticket.priority} status={ticket.status} />
                    </td>
                    <td className="px-6 py-4"><div className="text-white font-medium">{ticket.requester_name}</div><div className="text-xs text-gray-500">{ticket.sector} • {ticket.priority}</div><p className="text-gray-400 text-xs mt-1 line-clamp-1">{ticket.description}</p></td>
                    
                    {/* COLUNA DO TÉCNICO COM NOME VISÍVEL */}
                    <td className="px-6 py-4">
                        ...
                        ) : (showClaim ? (<button onClick={(e) => { e.stopPropagation(); claimTicket(ticket.id); }} className="bg-blue-600 hover:bg-blue-500 text-white text-xs px-3 py-1.5 rounded font-bold transition-all hover:scale-105">Assumir</button>) : <span className="text-gray-600 text-xs">--</span>)}
                    </td>

                    <td className="px-6 py-4">{ticket.technical_notes ? <FileText className="w-4 h-4 text-cyan-400" /> : <span className="text-gray-700">-</span>}</td>
                    <td className="px-6 py-4 text-center" onClick={(e) => e.stopPropagation()}><button onClick={() => deleteTicket(ticket.id)} className="text-gray-500 hover:text-red-400 p-2 hover:bg-red-500/10 rounded-lg"><Trash2 className="w-4 h-4" /></button></td>
                  </tr>
                ))
              )}
            </tbody>
  ...
  const renderLogin = () => (
    <div className="min-h-screen bg-[#020817] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#0f172a] border border-white/10 rounded-2xl p-8 text-center animate-in fade-in zoom-in">
        <div className="w-16 h-16 bg-cyan-500/10 rounded-full flex items-center justify-center mx-auto mb-6"><Lock className="w-8 h-8 text-cyan-400" /></div>
  ...
  const renderAdminLayout = () => (
    <div className="flex flex-col h-screen overflow-hidden bg-[#020817]">
      <header className="h-16 bg-[#0f172a] border-b border-white/10 flex items-center justify-between px-6 shrink-0">
        ...
        {/* HEADER MOSTRANDO O NOME DO TÉCNICO */}
        <div className="flex items-center gap-4">
            ...
            <button onClick={fetchTickets} className="text-gray-400 hover:text-white bg-white/5 p-2 rounded-lg"><RefreshCw className={`w-4 h-4 ${loadingTickets ? 'animate-spin' : ''}`}/></button>
            <button onClick={handleLogout} className="text-red-400 hover:bg-red-500/10 px-3 py-1.5 rounded-lg text-sm flex items-center gap-2"><LogOut className="w-4 h-4"/> Sair</button>
        </div>
           ...
           {adminTab === 'dashboard' && (
             <div className="space-y-6 animate-in fade-in">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-[#1e293b]/50 p-6 rounded-2xl border border-white/5 shadow-lg"><p className="text-gray-400 text-sm">Total</p><h3 className="text-4xl font-bold text-white mt-2">{stats.total}</h3></div>
                  <div className="bg-[#1e293b]/50 p-6 rounded-2xl border border-white/5 shadow-lg"><p className="text-gray-400 text-sm">Abertos</p><h3 className="text-4xl font-bold text-yellow-400 mt-2">{stats.open}</h3></div>
                  <div className="bg-[#1e293b]/50 p-6 rounded-2xl border border-white/5 shadow-lg"><p className="text-gray-400 text-sm">Resolvidos</p><h3 className="text-4xl font-bold text-green-400 mt-2">{stats.solved}</h3></div>
                </div>
                <div className="h-[300px] bg-[#1e293b]/50 p-4 rounded-2xl border border-white/5">
                   <ResponsiveContainer width="100%" height="100%"><BarChart data={stats.statusCount}><XAxis dataKey="name" stroke="#666"/><YAxis stroke="#666"/><Bar dataKey="value" fill="#0ea5e9" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer>
                </div>
             </div>
           )}
      ...
      {modalOpen && editingTicket && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-[#0f172a] border border-white/10 w-full max-w-2xl rounded-2xl shadow-2xl p-6">
             <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-white">Editar Chamado #{editingTicket.id}</h3><button onClick={() => setModalOpen(false)}><X className="text-gray-400"/></button></div>
             <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-4"><div><span className="text-xs font-bold text-gray-500 uppercase">Solicitante</span><p className="text-white text-lg">{editingTicket.requester_name}</p></div><div className="bg-white/5 p-4 rounded border border-white/5"><p className="text-gray-300 text-sm">{editingTicket.description}</p></div>{editingTicket.technician_email && <div className="text-xs text-cyan-400">Responsável: {editingTicket.technician_email}</div>}</div>
                <div className="space-y-4">
                   <label className="text-xs font-bold text-gray-500 uppercase">Status</label>
                   <select value={editingTicket.status} onChange={(e) => setEditingTicket({...editingTicket, status: e.target.value})} className="w-full bg-[#1e293b] border border-white/10 rounded-lg px-4 py-3 text-white outline-none"><option value="Aberto">🟢 Aberto</option><option value="Em Andamento">🟡 Em Andamento</option><option value="Concluído">⚪ Concluído</option></select>
                   <label className="text-xs font-bold text-gray-500 uppercase">Obs. Técnica</label>
                   <textarea value={editingTicket.technical_notes || ''} onChange={(e) => setEditingTicket({...editingTicket, technical_notes: e.target.value})} className="w-full bg-[#1e293b] border border-white/10 rounded-lg px-4 py-3 text-white outline-none h-32"></textarea>
                </div>
             </div>
             <div className="mt-6 flex justify-end gap-3"><button onClick={saveTicketChanges} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold px-6 py-2 rounded-lg flex items-center gap-2"><Save className="w-4 h-4"/> Salvar</button></div>
          </div>
        </div>
      )}
    </div>
        ...
        <div className="min-h-screen bg-[#020817] text-white font-sans flex flex-col">
          <nav className="border-b border-white/5 bg-[#0f172a]/80 backdrop-blur-md sticky top-0 z-50">
            <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
              ...
              <button onClick={() => handleNavigation('admin')} className="text-gray-400 hover:text-white flex items-center gap-2 transition-colors px-3 py-2 hover:bg-white/5 rounded-lg text-sm font-medium">Área Técnica <ExternalLink className="w-3 h-3" /></button>
            </div>
          </nav>
                ...
                {status === 'success' ? (
                  ...
                  <form onSubmit={handlePublicSubmit} className="space-y-5">
                    <div>
                      ...
                      <input required name="nome" value={formData.nome} onChange={handleFormChange} placeholder="Ex: Ana Souza" className="w-full bg-[#1e293b]/50 border border-white/10 rounded-lg px-4 py-3 text-white outline-none"/>
                    </div>
                    <div>
                        ...
                        <select required name="setor" value={formData.setor} onChange={handleFormChange} className="w-full bg-[#1e293b]/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all cursor-pointer">
                          ...
                          <option value="CF">Controle de Frota</option>
                          <option value="DRT">Diretoria</option>
                          <option value="FIN">Financeiro</option>
                          <option value="GRC">Gerencia</option>
                          <option value="HC">Home Care</option>
                          <option value="MTM">Monitoramento</option>
                          <option value="RH">Recursos Humanos</option>
                          <option value="RGL">Regulação</option>
                          <option value="TI">TI</option>
                        </select>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Prioridade</label>
                      <select name="prioridade" value={formData.prioridade} onChange={handleFormChange} className="w-full bg-[#1e293b]/50 border border-white/10 rounded-lg px-4 py-3 text-white outline-none"><option value="Baixa">Baixa</option><option value="Média">Média</option><option value="Alta">Alta</option></select>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Descrição</label>
                      <textarea required name="descricao" placeholder="Descrição..." rows="4" value={formData.descricao} onChange={handleFormChange} className="w-full bg-[#1e293b]/50 border border-white/10 rounded-lg px-4 py-3 text-white outline-none"></textarea>
                    </div>
